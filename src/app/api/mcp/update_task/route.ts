...
export async function POST(request: NextRequest) {
  ...
  if ('due_date' in bodyObj) {
    ...
    if (v === null) {
      ...
      if (trimmed && !/^\d{4}-\d{2}-\d{2}$/.test(trimmed)) {
        return NextResponse.json(
          { ok: false, error: 'due_date ma nieprawidlowy format (wymagane YYYY-MM-DD).' },
          { status: 400 }
        )
      }
      ...
      return NextResponse.json({ ok: false, error: 'due_date musi byc stringiem lub null.' }, { status: 400 })
    }
  ...
  if ('hidden' in bodyObj) {
    ...
    if (typeof v !== 'boolean') {
      return NextResponse.json({ ok: false, error: 'hidden musi byc boolean.' }, { status: 400 })
    }
  ...
  if ('kind' in bodyObj) {
    ...
    if (typeof v !== 'string' || !VALID_KINDS.includes(v as TaskKind)) {
      return NextResponse.json(
        { ok: false, error: `Nieprawidlowy kind: ${v}. Dozwolone: ${VALID_KINDS.join(', ')}.` },
        { status: 400 }
      )
    }
  ...
  if ('est' in bodyObj) {
    ...
    if (v !== null && typeof v !== 'number') {
      return NextResponse.json({ ok: false, error: 'est musi byc liczba lub null.' }, { status: 400 })
    }
  ...
  try {
    ...
    if (updErr) {
      ...
      return NextResponse.json({ ok: false, error: 'Nie udalo sie zaktualizowac zadania.' }, { status: 500 })
    }
