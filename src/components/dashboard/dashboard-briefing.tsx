...
import type { DashboardBriefData, UpcomingMilestone } from '@/lib/data/projects'
...
export function DashboardBriefing({ data, milestones = [] }: DashboardBriefingProps) {
      ...
      <div className="divide-y divide-border/50 flex flex-col">
              ...
              {atRiskProjects.slice(0, MAX_ITEMS).map((p, i) => (
                <li key={i}>
                  ...
                  >
                    ...
                    <ArrowRight className="h-3.5 w-3.5 text-muted-foreground/30 group-hover:text-muted-foreground ml-auto shrink-0 transition-colors" aria-hidden="true" />
                  </Link>
                </li>
              ))}
        ...
        {milestones.length > 0 && (
          <section className="py-3 first:pt-0 last:pb-0 flex flex-col gap-2" aria-labelledby="section-milestones">
            ...
            >
              <CalendarClock className="h-3.5 w-3.5 shrink-0" aria-hidden="true" />
              Kamienie milowe — 14 dni ({milestones.length})
            </h3>
            <ul className="flex flex-col gap-0.5" role="list">
              {milestones.slice(0, MAX_ITEMS).map((ms) => (
                <li key={ms.id}>
                  <Link
                  ...
                  >
                    <span className="font-meta text-sm text-foreground leading-snug flex-1 min-w-0 truncate">
                      {ms.msCode && (
                        <span className="text-teal font-semibold mr-1">{ms.msCode}</span>
                      )}
                      {ms.name}
                      <span className="text-muted-foreground/50 mx-1" aria-hidden="true">·</span>
              ...
              {milestones.length > MAX_ITEMS && (
                <li className="font-meta text-xs text-muted-foreground pl-3">
                  + {milestones.length - MAX_ITEMS} więcej
                </li>
              )}
            </ul>
          </section>
